...
public class Moritz implements TokenSource,
     ...
     *  argument is always <code>null</code>.
     */
    private ObserverList observersEOF = new ObserverList();

    /** This observer list is used for the observers which are registered
     *  to receive a notification when a new token is about to be delivered.
     *  The argument is the token to be delivered.
     */
    private ObserverList observersPop = new ObserverList();

    /** This observer list is used for the observers which are registered
     *  to receive a notification when a new token is pushed back to the input
     ...
     * The reading occurs on this new stream before resorting to the previous
     * streams.
     *
     ...
     * If all tokens are found then they are removed from the input stream and
    ...
    public Token scanNextNonSpace() throws GeneralException {
        for (Token t = scanNextToken(); t != null;
                 t = scanNextToken()) {
