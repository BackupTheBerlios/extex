...
public class Patterns extends AbstractCode {
	...
	public void execute(Flags prefix, Context context, TokenSource source, Typesetter typesetter) throws GeneralException {
		...
		while (st.hasMoreTokens()) {
			...
			ht.addPattern(createPatternName(pat, context), createPatternEntry(pat, context));
		}
	}
	 ...
	 * @return	the entry
	 */
	private String createPatternEntry(final String pattern, final Context context) {
	 ...
	 * @return	the name
	 */
	private String createPatternName(String pattern, Context context) {
		String rt = "";
		if (pattern != null) {
