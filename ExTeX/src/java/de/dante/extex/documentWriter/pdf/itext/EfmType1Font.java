/**
 * based on com.lowagie.text.pdf.Type1Font
 */

package de.dante.extex.documentWriter.pdf.itext;
 ...
 */
class EfmType1Font extends BaseFont {

    /**
     * @see com.lowagie.text.pdf.BaseFont#createFont(java.lang.String, java.lang.String, boolean)
     */
    public static BaseFont createFont(String name, String encoding,
        ...
        fontBuilt = new EfmType1Font(name, encoding, embedded, ttfAfm, pfb);
     ...
     */
    EfmType1Font(String afmFile, String enc, boolean emb, byte ttfAfm[],
            byte pfb[]) throws DocumentException, IOException {

        if (emb && ttfAfm != null && pfb == null)
            throw new DocumentException(
                    "Two byte arrays are needed if the Type1 font is embedded.");
        if (emb && ttfAfm != null)
            this.pfb = pfb;
        encoding = enc;
        embedded = emb;
        ...
        if (BuiltinFonts14.containsKey(afmFile)) {
            ...
            try {
                ...
                is = getResourceStream(RESOURCE_PATH + afmFile + ".afm",
                        resourceAnchor.getClass().getClassLoader());
                if (is == null) {
                    String msg = afmFile
            ...
            throw new DocumentException(afmFile
                    + " is not an AFM or PFM font file.");
        try {
     ...
     * @throws IOException the AFM file could not be read
     */
    public void process(RandomAccessFileOrArray rf) throws DocumentException,
            IOException {
        ...
        while ((line = rf.readLine()) != null) {
            ...
            if (ident.equals("FontName"))
                ...
                UnderlinePosition = (int) Float.valueOf(tok.nextToken())
                        .floatValue();
            else if (ident.equals("UnderlineThickness"))
                UnderlineThickness = (int) Float.valueOf(tok.nextToken())
                        .floatValue();
        ...
        while ((line = rf.readLine()) != null) {
            ...
            if (ident.equals("KPX")) {
                ...
                Integer width = new Integer(Float.valueOf(tok.nextToken())
                        .intValue());
    ...
    private PdfStream getFontStream() throws DocumentException {
        ...
        RandomAccessFileOrArray rf = null;
        try {
            String filePfb = fileName.substring(0, fileName.length() - 3)
                    + "pfb";
            if (pfb == null)
            ...
            for (int k = 0; k < 3; ++k) {
                if (rf.read() != 0x80)
                    throw new DocumentException("Start marker missing in "
                            + filePfb);
                if (rf.read() != pfbTypes[k])
                    throw new DocumentException("Incorrect segment type in "
                            + filePfb);
                int size = rf.read();
                size += rf.read() << 8;
                size += rf.read() << 16;
                ...
                while (size != 0) {
                    ...
                    if (got < 0)
                        throw new DocumentException("Premature end in "
                                + filePfb);
                    bytePtr += got;
                    size -= got;
                }
    ...
    private PdfDictionary getFontBaseType(PdfIndirectReference fontDescriptor,
            int firstChar, int lastChar, byte shortTag[]) {

        PdfDictionary dic = new PdfDictionary(PdfName.FONT);
        dic.put(PdfName.SUBTYPE, PdfName.TYPE1);
        dic.put(PdfName.BASEFONT, new PdfName(FontName));
        boolean stdEncoding = encoding.equals("Cp1252")
                || encoding.equals("MacRoman");
        if (!fontSpecific) {
            ...
            if (stdEncoding)
                dic.put(PdfName.ENCODING, encoding.equals("Cp1252")
                        ? PdfName.WIN_ANSI_ENCODING
                        : PdfName.MAC_ROMAN_ENCODING);
            else {
     ...
     */
    protected void writeFont(PdfWriter writer, PdfIndirectReference ref,
