 ...
 * If they are used up then the file stream is used to extract token by token.
...
public class TokenStreamImpl extends TokenStreamBufferImpl {
    ...
    public TokenStreamImpl(File filename, String encoding)
        ...
        decoder      = Charset.forName(encoding)
                              .newDecoder();
        refill();
    }
     ...
     */
    public TokenStreamImpl(Reader reader, String encoding)
                    throws IOException {
        super("", encoding);
     ...
     *
     * @return a new buffer of characters read or <code>null</code> at
     * end of file
     *
