 ...
 */
public abstract class AbstractTokenStreamImpl extends TokenStreamBaseImpl implements TokenStream, CatcodeVisitor {
	...
	//    /**
	//     * Set the buffer to a new character array and reset pointer and state.
	//     * @param cs the new buffer
	//     */
	//    protected void setBuffer(final CharBuffer cs) {
	//        buffer = cs;
	//        pointer = -1;// mgn: changed
	//        state = NEW_LINE;
	//    }
	 ...
	 */
	private int getChar(final Tokenizer tokenizer) throws MainIOException, GeneralException {
					...
					case 2 :
						...
						if (pointer < bufferLength()) {
							...
							if (hex >= 0) {
								...
								// 0100 = 64
								c = (char) ((hex < 0100) ? hex + 0100 : hex - 0100);
								pointer++;
							}
						} else {
