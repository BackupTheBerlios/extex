...
public abstract class Moritz implements TokenSource, Observable {
	...
	public long scanFloat() throws GeneralException {
		...
		if (t != null && (t.equals(Catcode.OTHER, ".") || t.equals(Catcode.OTHER, ","))) {
			...
			for (t = getToken(); t != null && t.isa(Catcode.OTHER) && t.getValue().matches("[0-9]"); t = scanToken()) {
				if (k < 17) {
	...
	public void update(String name, String text) throws NotObservableException {
		...
		//gene: this method can be specialized if only a single name is ever used.
	}
	 ...
	 */
	protected abstract Token expand(Token token) throws GeneralException;
	...
	/**
	 * Scans the input token stream for a given sequence of tokens.
	 * Those tokens may have the catcodes LETTER or OTHER.
	 * 
	 *
